...
- ポケモンタイプクイズ
- ポケモン一覧とタイプ確認
- AI ポケモン相談チャット（うろ覚えの特徴からポケモンを推測）
...
AI ポケモン相談チャット（`/chat`）は OpenAI API を利用します。`.env` などに以下を設定してください。

| 環境変数         | 内容                                                                                                              |
| ---------------- | ----------------------------------------------------------------------------------------------------------------- |
| `OPENAI_API_KEY` | OpenAI API キー。未設定の場合、AI チャットはエラーになります。                                                    |
| `CHAT_USE_MOCK`  | `1` を設定すると OpenAI を呼ばずに fixture をストリーミング返却する開発用モックモードになります（API キー不要）。 |
